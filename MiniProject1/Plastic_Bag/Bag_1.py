...
class Bag:
    ...
    def column_selection(self):
        ...
        if self.key_typed == 1:
            CC.basic_statistics(CC())  # Calls the basic_statistic method from bag_cc.py
        elif self.key_typed == 2:
            CC.total_bag(CC())  # Calls the total_bag method from bag_cc.py
        elif self.key_typed == 3:
            CC.net_vs_donation(CC())  # Calls the net_vs_donation method from bag_cc.py
        elif self.key_typed == 4:
            CC.vat_calc(CC())  # Calls the vat_calc method from bag_cc.py

    def graph_plotting(self):
        """Gives the user a selection on which graphs to plot"""
        if self.key_typed == 1:
            GP.pie_chart(GP())  # Calls the pie_chart method from bag_cc.py
        elif self.key_typed == 2:
            GP.bar_chart(GP())  # Calls the bar_chart method from bag_cc.py
        elif self.key_typed == 3:
            GP.line_graph(GP())  # Calls the line_graph method from bag_cc.py
        else:
