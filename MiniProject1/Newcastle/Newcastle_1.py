...
to process the Newcastle dataset
...
class Newcastle:
    ...
    def column_selection(self):
        ...
        if self.key_typed == 1:
            CC.basic_statistics(CC())  # Calls the basic_statistics method from newcastle_cc.py
        elif self.key_typed == 2:
            CC.total_electricity(CC())  # Calls the total_electricity method from newcastle_cc.py
        elif self.key_typed == 3:
            CC.difference_months(CC())  # Calls the difference_months method from newcastle_cc.py
        elif self.key_typed == 4:
            CC.percent_difference(CC())  # Calls the percent_difference method from newcastle_cc.py

    def graph_plotting(self):
        """Gives the user a selection on which graphs to plot"""
        if self.key_typed == 1:
            GP.pie_chart(GP())  # Calls the pie_chart method from newcastle_cc.py
        elif self.key_typed == 2:
            GP.bar_chart(GP())  # Calls the bar_chart method from newcastle_cc.py
        elif self.key_typed == 3:
            GP.line_graph(GP())  # Calls the line_graph method from newcastle_cc.py
        else:
...
def main():
    ...
    if Newcastle.column_or_graph() is True:
        return Newcastle.column_selection(Newcastle())
    return Newcastle.graph_plotting(Newcastle())
